...
namespace PolyMesh
{
    public static class Settings
    {
        ...
        public const string path = @"C:..\\..\\..\\fulltorus.obj";
        public const int bitmapSize = 800;
        public const int modelScale = 1;
    ...
    public class Triangle
    {
        ...
        public void Paint(DirectBitmap bits, Color color, bool kaOnly = false)
        {
            ResetEdges();
            ...
            for (int y = (int)ymin; y < (int)ymax; y++)
            {
                ...
                if (AET.Count > 0)
                {
                    FillAET(bits, AET, y, color, kaOnly);
                }
            }
        }
        private void FillAET(DirectBitmap bits, List<AETP> AET, int y, Color color, bool kaOnly = false)
        {
            ...
            {
                FillLine(bits, AET[i].x, AET[i + 1].x, y, color, kaOnly);
            }
        ...
        private void FillLine(DirectBitmap bits, int x1, int x2, int y, Color modelColor, bool lab = false, bool kaOnly = false)
        {
            for (int x = x1; x <= x2; x++)
            {
                ...
                if (lab)
                {
                    var pos = vertices.Select(p => p.position).ToArray();
                    (float w1, float w2, float w3) weights = Geometry.GetBarycentricWeights(pos, x, y);
                    var Z = weights.w1 * vertices[0].position.Z + weights.w2 * vertices[1].position.Z + weights.w3 * vertices[2].position.Z;
                    if(Geometry.zBuffer[x,y] >= Z)
                    {
                        Geometry.zBuffer[x, y] = Z;
                        bits.SetPixel(x, y, modelColor);
                    }
                    continue;
                }
                if (kaOnly)
        ...
        public void PaintOld(DirectBitmap bits, Color color)
        {
            ...
            for (int i = ymin; i <= ymax; i++)
            {
                ...
                //var scan2 = scan.Select(x => Math.Abs(x.v2.position.Y - x.v1.position.Y) > 0.01 ? (int)((i - x.v1.position.Y) * (x.v2.position.X - x.v1.position.X) / (x.v2.position.Y - x.v1.position.Y) + x.v1.position.X) : Math.Min((int)x.v1.position.X, (int)x.v2.position.X)).ToArray();
                var scan2 = scan.Select(x => ((i - x.v1.position.Y) * (x.v2.position.X - x.v1.position.X) / (x.v2.position.Y - x.v1.position.Y) + x.v1.position.X)).ToArray();
                Array.Sort(scan2);
                if (scan2 == null || scan2.Length == 0) continue;
                for (int j = (int)scan2[0]; j < scan2[^1]; j++)
                {
